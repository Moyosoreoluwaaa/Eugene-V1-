...
object SeedDataProvider {
    ...
    fun getDefaultSnapshots(): List<SecondingSnapshotDto> {
        val baseInstant = Instant.parse("2026-07-16T00:00:00Z")
        return listOf(
            SecondingSnapshotDto(
                predictionId = "pred_no_rollup",
                timestamp = baseInstant,
                totalSeconds = 1000,
                outcomes = listOf(
                    SecondingSnapshotOutcomeDto("opt_noroll_yes", 20, 200),
                    SecondingSnapshotOutcomeDto("opt_noroll_no", 80, 800)
                )
            ),
            SecondingSnapshotDto(
                predictionId = "pred_no_rollup",
                timestamp = Instant.fromEpochMilliseconds(baseInstant.toEpochMilliseconds() + 5 * 60 * 1000L), // 5 min later
                totalSeconds = 1200,
                outcomes = listOf(
                    SecondingSnapshotOutcomeDto("opt_noroll_yes", 25, 300),
                    SecondingSnapshotOutcomeDto("opt_noroll_no", 75, 900)
                )
            ),
            SecondingSnapshotDto(
                predictionId = "pred_no_rollup",
                timestamp = Instant.fromEpochMilliseconds(baseInstant.toEpochMilliseconds() + 10 * 60 * 1000L), // 10 min later
                totalSeconds = 1500,
                outcomes = listOf(
                    SecondingSnapshotOutcomeDto("opt_noroll_yes", 30, 450),
                    SecondingSnapshotOutcomeDto("opt_noroll_no", 70, 1050)
                )
            )
        )
    }
